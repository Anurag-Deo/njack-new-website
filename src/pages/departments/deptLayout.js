...
const DeptDescription = ({ deptDesc }) => {
	return (
		<div className={styles.deptDesc}>
			...
			<DisplayLottie animationPath={'https://assets7.lottiefiles.com/packages/lf20_iv4dsx3q.json'} />
			</div>
			<p>{deptDesc}</p>
		</div>
	)
			...
			<InView>
						{({ inView, ref }) => (
							<motion.div className={styles.cardSection} ref={ref}>
								{eventsArr?eventsArr.map((event, index) => (
									//   {event.old === false && (
									<motion.div
										key={event.key}
	...
	return (
		<div className={styles.section}>
			...
			<InView>
				{({ inView, ref }) => (
					<motion.div className={styles.cardSectionTeams} ref={ref}>
						{coordArr ? coordArr.map((member, index) => (
							<motion.div
								key={member.name}
								className={styles.card}
			...
			<InView>
				{({ inView, ref }) => (
					<motion.div className={styles.cardSectionTeams} ref={ref}>
						{subCoordArr ? subCoordArr.map((member, index) => (
							<motion.div
								key={member.name}
								className={styles.card}
								initial={{ opacity: 0, y: 20 }}
